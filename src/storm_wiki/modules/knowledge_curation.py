...
from storm_wiki.modules.persona_generator_local import LocalCreateWriterWithPersona
...
class ConvSimulator(dspy.Module):
    ...
    def __init__(
        ...
        self.topic_expert = LocalTopicExpert(
...
class QuestionToQuery(dspy.Signature):
    ...
    ...
    - query n: x\n
    where x represents the query you want to type in, do not output any other content.
    """
...
class LocalTopicExpert(dspy.Module):
    """Answer questions using search-based retrieval and answer generation. This module conducts the following steps:
    1. Generate queries from the question.
    2. Search for information using the queries.
    3. Filter out unreliable sources.
...
class StormKnowledgeCurationModule(KnowledgeCurationModule):
    ...
    def __init__(
        self,
        retriever: Retriever,
        persona_generator: Optional[LocalCreateWriterWithPersona],
