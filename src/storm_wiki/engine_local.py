...
from storm_wiki.modules.persona_generator_local import LocalStormPersonaGenerator
from storm_wiki.modules.retriever import StormRetriever, localStormRetriever
    ...
    def init_openai_model(
        ...
        if openai_type and openai_type == "azure":
            ...
            self.outline_gen_lm = OpenAIModel(
                model="gpt-4", engine="gpt-4", max_tokens=400, **openai_kwargs
            )
            self.article_gen_lm = OpenAIModel(
                model="gpt-4", engine="gpt-4", max_tokens=700, **openai_kwargs
            )
            self.article_polish_lm = OpenAIModel(
                model="gpt-4-32k", engine="gpt-4-32k", max_tokens=4000, **openai_kwargs
            )
...
class STORMWikiRunner(Engine):
    ...
    def __init__(
        ...
        self.retriever = localStormRetriever(rm=rm, k=self.args.retrieve_top_k)
        storm_persona_generator = LocalStormPersonaGenerator(
            self.lm_configs.question_asker_lm, rm=rm
        )
